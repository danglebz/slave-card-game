...
test('เลี้ยงกาแฟ: เปิด modal แล้ว QR พร้อมเพย์เรนเดอร์ได้ + โชว์เบอร์ผู้รับที่ถูกต้อง', async ({
  ...
  await expect(modal.locator('.support-qr img')).toHaveAttribute('src', /^data:image\/png;base64,/);

  // เบอร์ผู้รับต้องโชว์ถูกต้องทั้งบนแผ่น QR และปุ่มคัดลอก — ผิดตัวเดียว = เงินเข้าคนอื่น
  await expect(modal.locator('.support-qr-brand')).toHaveText('PromptPay · 085-796-8525');
  await expect(modal.locator('.support-id')).toContainText('085-796-8525');

  // ปุ่มจ่ายด้วยบัตรต้องชี้ไปหน้า Ko-fi ที่ถูกต้อง — ผิด URL = เงินไปเข้าคนอื่นเหมือนกัน
  await expect(modal.locator('.support-card-pay a')).toHaveAttribute(
    'href',
